...
/usr/bin/chromium-browser --kiosk --window-position=-1,0 --window-size=1024,768 --start-fullscreen --incognito  --noerrdialogs --disable-translate --no-first-run --fast --fast-start --disable-infobars --disable-features=TranslateUI --disk-cache-dir=/dev/null --ignore-certificate-errors https://kevin.lvk.sh/
